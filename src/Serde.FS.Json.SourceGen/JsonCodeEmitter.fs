...
module internal JsonCodeEmitterImpl =
    ...
    let emitCustom (info: SerdeTypeInfo) (converterFqn: string) : string =
        ...
        appendf "        inherit JsonConverter<%s>()" fqn
        appendf "        override _.Read(reader, _typeToConvert, options) ="
        appendf "            let node = JsonSerializer.Deserialize<System.Text.Json.Nodes.JsonNode>(&reader, options)"
        appendf "            let c = %s()" converterFqn
        appendf "            (c :> Serde.FS.Json.ISerdeConverter<%s>).Deserialize(node)" fqn
        appendf "        override _.Write(writer, value, options) ="
        appendf "            let c = %s()" converterFqn
        appendf "            let node = (c :> Serde.FS.Json.ISerdeConverter<%s>).Serialize(value)" fqn
        appendf "            node.WriteTo(writer)"
        append ""
        appendf "    let %s (options: JsonSerializerOptions) : JsonTypeInfo<%s> =" fnName fqn
        appendf "        JsonMetadataServices.CreateValueInfo<%s>(options, %s())" fqn converterName
