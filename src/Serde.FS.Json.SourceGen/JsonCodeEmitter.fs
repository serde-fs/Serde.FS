...
module internal JsonCodeEmitterImpl =
    ...
    let emitConsolidatedFile (types: SerdeTypeInfo list) : string option =
        ...
        | _ ->
            ...
            // When two types share a short TypeName (e.g. CEI.Domain.Forge.Project
            // and CEI.Domain.Project.Project both produce `projectJsonCodec`),
            // their inlined let-bindings collide in this single module. Detect
            // those collisions and produce a disambiguated function name for
            // each colliding info — built from EnclosingModules + TypeName so
            // `Forge.Project` becomes `forgeProjectJsonCodec` and
            // `Project.Project` becomes `projectProjectJsonCodec`. Non-colliding
            // types keep their plain `<typeName>JsonCodec` form so existing
            // snapshots are unaffected.
            let baseNameOf (info: SerdeTypeInfo) = resolverFnName info
            let collisionCounts =
                types
                |> List.countBy baseNameOf
                |> Map.ofList
            let isColliding (info: SerdeTypeInfo) =
                Map.tryFind (baseNameOf info) collisionCounts
                |> Option.exists (fun c -> c > 1)
            let disambiguatedFnName (info: SerdeTypeInfo) =
                if isColliding info then
                    let parts = info.Raw.EnclosingModules @ [ info.Raw.TypeName ]
                    let pascal = parts |> List.map upperFirst |> String.concat ""
                    lowerFirst pascal + "JsonCodec"
                else baseNameOf info
            ...
            // Emit all codec bodies inline. For colliding names, rewrite the
            // top-level `let private <oldFn>` to the disambiguated form so
            // the binding name matches what the registration code references.
            for info in types do
                let fullCode = emit info
                let body = extractCodecBody fullCode
                let body =
                    if isColliding info then
                        let oldFn = baseNameOf info
                        let newFn = disambiguatedFnName info
                        body.Replace($"let private {oldFn} ", $"let private {newFn} ")
                    else body
                append body
                append ""
            ...
            for info in types do
                if isGenericSingleCaseWrapper info then
                    ...
                    let fnName = disambiguatedFnName info
